...
my $version = '6.04.00';	# Current version number - should match sa6package.txt in the current scan package
...
################################################################################
{
#&DownloadFileID( "applications", undef, undef, "fileid.xml", undef );
#die;
...
################################################################################
{
	...
	my $scan_tmp_dir	= $working_dir . "\\ScanTemp";
	...
	if ( -d $scan_tmp_dir )
		{	print "Removing ScanTemp directory $scan_tmp_dir ...\n";
			my $cmd = "rmdir \"$scan_tmp_dir\" \/s \/q";
			system( $cmd );
