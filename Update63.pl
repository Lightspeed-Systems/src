...
my $version = '6.03.00';	# Current version number - should match sa6package.txt in the current scan package
...
################################################################################
{
	...
	if ( $opt_install )
		{	&SecurityLogEvent( "Update: Installing a new scan engine package from the command line\n" );
			...
			my $full_filename = "SA63Update.htm";
			$full_filename = $dir . "\\SA63Update.htm" if ( $dir );
...
{	my $uninstall = shift;
	...
	if ( ! -e "lsguide.dll" )
		{	&SecurityLogEvent(  "Update: Error - unable to find lsguide.dll in SA directory.\n" );
			&RegCloseKey( $key );
			return( undef );
		}
	...
	if ( $len <= 0 )
		{	&SecurityLogEvent(  "Update: Unregistering Guide LSP in SA directory.\n" );
			system "LsRegister.exe /unregserver";
		...
		{	# Set the status flags
			...
			&SecurityLogEvent(  "Update: Key exists and the Guide LSP should be registered\n" ) if ( $lsp_registered );
			&SecurityLogEvent(  "Update: Key exists and the Guide LSP should NOT be registered\n" ) if ( ! $lsp_registered );
		}
	...
	if ( $should_use_lsp )
		{	&SecurityLogEvent(  "Update: Registering Guide LSP in SA directory.\n" );
			system "lsregister /regserver";
		}
	else
		{	&SecurityLogEvent(  "Update: Unregistering Guide LSP in SA directory.\n" );
			system "lsregister /unregserver";
		}
...
{	my $force_update_now = shift;	# True if I should update now, no matter what the time
	...
	if ( $opt_reload )
		...
		{	my $update_now;	# Set this to true if it is time to update now
			...
			my $expired = &ExpiredTime( $update_interval, $last_update );
			...
			if ( ( $update_now )  &&  
				...
				( ! $changed )  &&  
				( $update_time ) )
				{	# Make sure that the update time is rational
					$update_time = 0 + 12 if ( ( $update_time < 0 )  ||  ( $update_time > 23 ) );
	...
	# Make sure that the file integrity file at least contains my programs
	my $signaled = &UpdateSecurityAgentFileIntegrity();
	$signal_service = undef if ( $signaled );	# If I just signaled the service, don't do it again unless I have to
...
# end of superceded section	
	...
	if ( $batch_results )
		{	my $last_attribute_report;
			...
			# Expired time returns 2 if it's been a long time, 1 if it need a normal update, or undef if current
			$report_attributes = &ExpiredTime( "Day", $last_attribute_report ) if ( ! $report_attributes );
...
################################################################################
{
	...
	my $working_dir		= &ScanWorkingDirectory();
	my $system_dir		= &ScanSystemDirectory();
	my $tmp_dir			= $working_dir . "\\tmp";
	my $quarantine		= $working_dir . "\\quarantine";
	...
	my $log_dir	= &ScanWorkingDirectory() . "\\log";
	if ( -d $log_dir )
...
{	my $key;
	...
	if ( ( $ok )  &&  ( $len > 0 ) )
		{	$scan_type = "quick";
			$scan_type = "full" if ( ( $data )  &&  ( $data eq "full" ) );
			$save_properties = 1 if ( ! $data );
		}
	$save_properties = 1 if ( ( ! $ok )  ||  ( $len <= 0 ) );
...
{	my $ttc_server	= shift;
	...
	my $url = "http:\/\/TTCSERVER\/content\/GetFileInfo.aspx?FileName=SA63Update.htm";
	...
	if ( $current_date ne $last_update )
		{	$changed = 1;
			...
			# See if I can read the sa6package.txt file and get the version from that ...
			$url = "http:\/\/TTCSERVER\/content\/SA63Package.txt";
			$url =~ s/TTCSERVER/$local_ttc_server/;
	...
	if ( $changed )	
		{	my $dir = &ScanWorkingDirectory();
			...
			$url = "http:\/\/TTCSERVER\/contentfiltering\/SA63Update.htm";
			$url =~ s/TTCSERVER/$local_ttc_server/;
			
			my $full_filename = $dir . "\\SA63Update.htm";
...
#
#  Given the SA6Update scan engine package, install it
...
{	my $dir				= shift;
	...
	# Look for the sa6package.txt file first so that is in the Security Log first ...
	my $package_version;
	...
	if ( ( ! $errors )  &&  ( $ttc_server_mode ) )
		{	my $software_dir = &SoftwareDirectory();
			...
			$new_filename = $software_dir . "\\Website\\Content\\SA6Update.htm";
			&ScanNoReadOnly( $new_filename );
			
			if ( copy( "SA6Update.htm", $new_filename ) )
			...
			$new_filename = $software_dir . "\\Website\\Content\\sa6package.txt";
			&ScanNoReadOnly( $new_filename );
			
			if ( copy( "sa6package.txt", $new_filename ) )
	...
	# Save the sa6package.txt into the registry
...
#
#  Return True if I had to signal the Security Agent service
#
################################################################################
{
...
sub CleanUpFiles( $$ )
#
#  Clean up all the files created when installing a new SA6Update
#
################################################################################
{	my $dir		= shift;
...
{	my $last_scan_started	= shift;	# The last time I scanned - in system time (secs)
	...
	if ( &IsScanRunning() )
		{	&SecurityLogEvent( "Update: The scan is already running\n" );
	...
	if ( ( $diff_hours < ( 0 + 12 ) )  &&  ( ! $scan_now ) )
		{	# Round it off
			...
			&SecurityLogEvent( "Update: Not scanning now because the last scan was only $diff_hours ago\n" );
			return( undef, $last_purge_started );	
		}
	...
	if ( ( $current_hour < $scan_hour )  &&  ( ! $scan_now ) )
		{	# Format the time so that it prints pretty
			...
			&SecurityLogEvent( "Update: Not scanning now because the current time is $current and the scan start time is set to $scan_start\n" );
			return( undef, $last_purge_started );	
		}
	...
	# Add all unknown files into the file integrity database
	my $cmd;
	...
	# Should I block spyware?
	$cmd = $cmd . " -k" if ( $block_spyware );
	
	# Should I send unknowns to Lightspeed?
	$cmd = $cmd . " -l" if ( $lightspeed_analyze_unknown );
	...
	my $outgoing_process;	
	$ok = Win32::Process::Create( $outgoing_process, $full_filename, $cmd, 0, NORMAL_PRIORITY_CLASS, $dir );
	if ( ! $ok )
			{	my $str = Win32::FormatMessage( Win32::GetLastError() );
...
use constant {
	...
	if ( ! $fwMgr )
		{	my $err_msg = Win32::OLE->LastError();
			...
			&SecurityLogEvent( "Update Error: OLE error getting the current Windows firewall exception list: $err_msg.\n" );
			return( undef );
		}
	...
	if ( ! $profile )
		{	my $err_msg = Win32::OLE->LastError();
			$err_msg = "Unknown" if ( ! $err_msg );
			&SecurityLogEvent( "Update Error: OLE error getting the current local firewall policy profile: $err_msg.\n" );
			return( undef );
		}
