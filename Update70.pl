...
my $version = '7.02.05';	# Current version number - should match sa8package.txt, sa72package.txt, sa7package.txt, sa6package.txt, or sa7package64.txt in the current scan package
...
################################################################################
{
	...
	if ( ! $ttc_server )
		...
		{	&SecurityLogEvent( "Update: Using $ttc_server as the update source\n" );
			...
			# Now report everything that I found out
			my ( $report_ok, $redirect_ttc_server ) = &ReportSecurityAgent( $ttc_server, $computer_name, $computer_domain, $comment, $os, $version, $username, $group_list, $user_ou );
			...
			if ( ( $report_ok )  &&  ( $redirect_ttc_server )  &&  ( lc( $ttc_server ) ne ( lc( $redirect_ttc_server ) ) ) )
				{	&SecurityLogEvent( "Update: TTC Server $ttc_server redirected this PC to use another TTC Server named $redirect_ttc_server\n" );
					...
					my ( $redirect_ok, $new_redirect_ttc_server ) = &ReportSecurityAgent( $redirect_ttc_server, $computer_name, $computer_domain, $comment, $os, $version, $username, $group_list, $user_ou );	
		...
		{	my $update_now;	# Set this to true if it is time to update now
			...
			if ( ! $update_now )
				{	&SecurityLogEvent( "Update: The last complete update was $last_update.\n" ) if ( $last_update );
					...
					$security_agent_signal_service = 1 if ( &SAImportCritical( $ttc_server, $opt_reload, \$signature_update, \$banned_update, \$category_update, \$integrity_update, \$fileid_update, \$registry_update ) );
